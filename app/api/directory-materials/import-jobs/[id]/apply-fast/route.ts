...
import { parseDirectoryMaterialId, parseDirectoryMaterialImportApplyBody } from "@/features/directory-materials/server/directory-materials.schemas"
...
function handleError(err: unknown) {
  ...
  if (err instanceof ZodError) return jsonError("BAD_REQUEST", err.issues[0]?.message ?? "Некорректные параметры запроса", 400)

  console.error("[POST /api/directory-materials/import-jobs/[id]/apply-fast]", err)
  return jsonError("INTERNAL_ERROR", "Ошибка быстрого применения импорта материалов", 500)
}
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    revalidateTag(directoryMaterialsCacheTags.list(context.workspaceOwnerId), "max")
    revalidateTag(directoryMaterialsCacheTags.categories(context.workspaceOwnerId), "max")
    revalidateTag(directoryMaterialsCacheTags.importJob(context.workspaceOwnerId, response.data.job.id), "max")
    revalidateTag(directoryMaterialsCacheTags.aiSearchIndex(context.workspaceOwnerId), "max")
