...
function invitationAcceptError(reason: string) {
  switch (reason) {
    ...
    case 'no_invitation_metadata':
      return jsonError('INVITATION_REQUIRED', 'Приглашение не найдено в текущей сессии', 400)
    case 'not_found':
      return jsonError('INVITATION_NOT_FOUND', 'Приглашение не найдено или уже использовано', 404)
    case 'expired':
      return jsonError('INVITATION_EXPIRED', 'Срок действия приглашения истёк', 410)
    case 'email_mismatch':
      return jsonError('INVITATION_EMAIL_MISMATCH', 'Приглашение предназначено для другого email', 403)
    default:
      return jsonError('INVITATION_ACCEPT_FAILED', 'Не удалось принять приглашение', 400)
  }
}
