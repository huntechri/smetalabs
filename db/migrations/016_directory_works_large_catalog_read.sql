-- ============================================================================
-- Migration: 016_directory_works_large_catalog_read
-- Description: Optimize directory works read/search paths for 100k-row catalogs.
-- Date: 2026-05-14
-- ============================================================================

-- Targeted exact-code/source and normalized filter indexes used before fuzzy
-- ranking. These keep the common q=<code> and filtered browse paths index-led.
CREATE INDEX IF NOT EXISTS idx_directory_works_active_workspace_code_lower
  ON public.directory_works(workspace_owner_id, lower(code), updated_at DESC, id)
  WHERE status = 'active' AND deleted_at IS NULL AND code IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_directory_works_active_workspace_source_key_lower
  ON public.directory_works(workspace_owner_id, lower(source_external_row_key), updated_at DESC, id)
  WHERE status = 'active' AND deleted_at IS NULL AND source_external_row_key IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_directory_works_active_workspace_norm_category_unit
  ON public.directory_works(
    workspace_owner_id,
    private.directory_work_normalize(category),
    private.directory_work_normalize(coalesce(subcategory, '')),
    private.directory_work_normalize(unit_code),
    updated_at DESC,
    id
  )
  WHERE status = 'active' AND deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_directory_works_active_workspace_norm_unit_label
  ON public.directory_works(workspace_owner_id, private.directory_work_normalize(unit_label), updated_at DESC, id)
  WHERE status = 'active' AND deleted_at IS NULL;
...
DECLARE
  ...
  -- Keep the legacy numeric cursor contract. Interactive API parsing caps this
  -- before the RPC call; export remains chunk-bounded at the application layer.
...
BEGIN
  ...
  -- Empty browse/list: no relevance ranking, no alias/keyword aggregation and no
  -- count(*) window. It relies on partial workspace/order/filter indexes.
  ...
  -- Exact code/source key: return the targeted indexed hit set only. This avoids
  -- broad trigram ranking when users paste a catalog code or external row key.
  ...
  WITH work_candidates AS (
    ...
    SELECT id, max(candidate_rank) AS candidate_rank
    FROM (
