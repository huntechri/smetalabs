-- ============================================================================
-- Migration: 018_directory_works_staged_search
-- Description: Search across the full workspace result set, then page output.
-- Date: 2026-05-14
-- ============================================================================
...
BEGIN
  ...
  -- Plain browse: page the full workspace/filter result set. Do not run search
  -- ranking or total count here.
  ...
  -- Exact code/source-key search stays first and searches the whole workspace,
  -- not the current UI page.
  SELECT EXISTS (
    SELECT 1
    FROM public.directory_works w
    WHERE w.workspace_owner_id = p_workspace_owner_id
  ...
  -- Fast text search: prefix/fts/alias/keyword over the whole workspace/filter
  -- set. Pagination is applied after all matches are found.
  SELECT EXISTS (
    SELECT 1
    FROM public.directory_works w
    WHERE w.workspace_owner_id = p_workspace_owner_id
    ...
    WHERE wa.workspace_owner_id = p_workspace_owner_id
      AND wa.deleted_at IS NULL
      AND (wa.normalized_alias = v_normalized_q OR wa.normalized_alias LIKE v_normalized_q || '%')
    UNION ALL
    SELECT 1
    FROM public.work_keywords wk
    JOIN public.directory_works w
    ...
    WHERE wk.workspace_owner_id = p_workspace_owner_id
      AND wk.deleted_at IS NULL
      AND (wk.normalized_keyword = v_normalized_q OR wk.normalized_keyword LIKE v_normalized_q || '%')
    LIMIT 1
  ...
  WITH work_candidates AS (
    ...
    SELECT
      ...
      max((
        ...
        + CASE WHEN NOT v_has_fast_match THEN extensions.similarity(wa.normalized_alias, v_normalized_q) * 90 ELSE 0 END
      )::double precision) AS candidate_rank
    FROM public.work_aliases wa
    JOIN public.directory_works w
      ON w.id = wa.work_id
    ...
    WHERE wa.workspace_owner_id = p_workspace_owner_id
      ...
      AND (
        ...
        OR (NOT v_has_fast_match AND extensions.similarity(wa.normalized_alias, v_normalized_q) >= 0.18)
      )
    GROUP BY wa.work_id
    ...
    SELECT
      ...
      max((
        ...
        + CASE WHEN NOT v_has_fast_match THEN extensions.similarity(wk.normalized_keyword, v_normalized_q) * 90 ELSE 0 END
      )::double precision) AS candidate_rank
    FROM public.work_keywords wk
    JOIN public.directory_works w
      ON w.id = wk.work_id
    ...
    WHERE wk.workspace_owner_id = p_workspace_owner_id
      ...
      AND (
        ...
        OR (NOT v_has_fast_match AND extensions.similarity(wk.normalized_keyword, v_normalized_q) >= 0.18)
      )
    GROUP BY wk.work_id
  ), candidate_ids AS (
