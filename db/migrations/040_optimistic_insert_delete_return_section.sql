-- ============================================================================
-- Migration: 040_optimistic_insert_delete_return_section
-- Description: Modify 6 RPC functions to RETURN affected section as jsonb
--   instead of uuid/void. Eliminates client-side read-after-write via
--   getProjectEstimateContentSectionForWorkspace for insert/delete ops.
-- Date: 2026-05-21
-- ============================================================================

-- Drop old versions first (return type changed)
DROP FUNCTION IF EXISTS public.create_estimate_section(uuid, uuid, uuid, text, uuid);
...
DROP FUNCTION IF EXISTS public.archive_estimate_section(uuid, uuid, uuid, uuid, uuid);
DROP FUNCTION IF EXISTS public.archive_estimate_work(uuid, uuid, uuid, uuid, uuid);
DROP FUNCTION IF EXISTS public.archive_estimate_material(uuid, uuid, uuid, uuid, uuid);

-- ----------------------------------------------------------------------------
-- 1. create_estimate_section: uuid → jsonb
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 2. add_work_from_directory_to_estimate: uuid → jsonb
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 3. add_material_from_directory_to_estimate: uuid → jsonb
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4. archive_estimate_section: void → jsonb (returns NULL — section deleted)
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Section deleted — no re-read data, return NULL
  RETURN NULL;
END;
$$;
...
-- ----------------------------------------------------------------------------
-- 5. archive_estimate_work: void → jsonb
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 6. archive_estimate_material: void → jsonb
-- ----------------------------------------------------------------------------
...
DECLARE
  ...
  v_now timestamptz := now();
BEGIN
  SELECT pem.id, pem.section_id
  INTO v_material
  FROM public.project_estimate_materials pem
  WHERE pem.workspace_owner_id = p_workspace_owner_id
    AND pem.project_id = p_project_id
    AND pem.estimate_record_id = p_estimate_record_id
    AND pem.id = p_material_id
  ...
  RETURN (
    ...
    FROM public.project_estimate_sections s
    JOIN public.project_estimate_records r ON r.id = s.estimate_record_id
    WHERE s.id = v_material.section_id
  );
END;
$$;
