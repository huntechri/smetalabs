...
AS $$
  ...
  plan AS (
    SELECT
      ...
      CASE
        WHEN SUM(pem.quantity) > 0
        THEN SUM(pem.quantity * pem.price) / SUM(pem.quantity)
        ELSE AVG(pem.price)
      END AS plan_price,
  ...
  fact AS (
    SELECT
      ...
      CASE
        WHEN COALESCE(ef.quantity, 0) + COALESCE(gf.quantity, 0) > 0
        THEN (COALESCE(ef.total, 0) + COALESCE(gf.total, 0)) / (COALESCE(ef.quantity, 0) + COALESCE(gf.quantity, 0))
        ELSE NULL
      ...
      END AS source,
      CASE
        WHEN ef.purchase_id IS NOT NULL AND gf.directory_material_id IS NULL THEN true
        ELSE false
      END AS editable
  ...
  FROM plan
  FULL OUTER JOIN fact ON plan.directory_material_id IS NOT DISTINCT FROM fact.directory_material_id
  ORDER BY title;
$$;
