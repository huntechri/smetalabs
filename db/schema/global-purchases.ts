...
export const globalPurchases = pgTable(
  "global_purchases",
  {
    ...
    planQuantity: numeric("plan_quantity", { precision: 14, scale: 3 }).notNull(),
  ...
  (t) => [
    uniqueIndex("uq_global_purchases_id_workspace").on(t.id, t.workspaceOwnerId),
    ...
    check("chk_global_purchases_plan_quantity_non_negative", sql`${t.planQuantity} >= 0`),
    check("chk_global_purchases_plan_price_non_negative", sql`${t.planPrice} >= 0`),
    check(
