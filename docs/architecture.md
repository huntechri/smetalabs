...
| Клиент БД | `@supabase/supabase-js` | 2.105.4 | Прямые запросы к БД (без ORM) в рантайме |
| Схема и Миграции | Drizzle ORM | 0.45.2 | Описание схемы БД и локальные миграции |
| Клиентский кэш | `@tanstack/react-query` | 5.100.10 | Запросы, мутации и кэширование во фронтенде |
...
**Что НЕ используется:**
- ❌ Prisma — не используется в проекте
- ❌ Реляционные запросы ORM в API — Drizzle используется только для миграций и типов схем, запросы выполняются через Supabase SDK.
...
Каждый бизнес-домен — папка в `features/`. В рамках архитектурного рефакторинга по разделению слоёв ([эпик #196](https://github.com/huntechri/smetalabs/issues/196)) все модули переводятся на строгое четырёхслойное деление:

```
features/{domain}/
├── ui/                                 # Слой отображения: компоненты, формы, разметка
├── model/                              # Чистый домен: типы, константы, чистые вычисления и мапперы
├── application/                        # Сценарии: React-хуки, TanStack Query, оркестрация действий
└── api/                                # Доступ к данным: Supabase-запросы, RPC-вызовы, адаптеры
```

Подробное описание ответственности каждого слоя, правила импорта и чек-лист для проверки PR описаны в отдельном документе:
👉 **[Архитектурный стандарт: Разделение слоёв в feature-модулях](./feature-layers-standard.md)**

**Основные правила написания кода:**
- **Один компонент = один файл**, именованный экспорт.
- Компоненты — только JSX + пропсы, без тяжелой бизнес-логики и прямых обращений к БД.
- Хуки — на своем уровне в слое `application/` или `ui/` (для локального стейта), отдельно от рендеринга.
- Моки — временные, удаляются при полной интеграции с базой данных.
...
1. **Схема базы данных** и типы описываются через **Drizzle ORM** в каталоге `db/schema/`.
2. **Локальные миграции** генерируются и управляются через **Drizzle Kit** (`pnpm db:generate`, `pnpm db:migrate`) и хранятся в репозитории в каталоге `db/migrations/`.
3. **Запросы к БД** в рантайме (Route Handlers, Server Actions, репозитории фич) выполняются напрямую через Supabase SDK (`@supabase/supabase-js`), используя анонимный или сервисный клиент (в обход RLS для серверных операций после ручной валидации прав).
4. **Клиентский слой** активно использует **TanStack Query** (React Query) для выполнения асинхронных запросов, инвалидации кэша и оптимистичных обновлений интерфейса.
...
Все таблицы в БД имеют `rls_enabled: true`. Политики определены в миграциях (миграции 002, 004, 005, 006, 007, 008, 014). Доступ к данным ограничен через `workspace_owner_id` и ролевые проверки.

---

## Миграции БД (Drizzle & Supabase)

Миграции генерируются с помощью `drizzle-kit` и хранятся локально в репозитории в каталоге `db/migrations/`. Они применяются к базе данных PostgreSQL (Supabase) через CLI. Всего на текущий момент определено 45 миграций (от `002_rls_policies` до `044_estimate_materials_ceil_quantity`).
