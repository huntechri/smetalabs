...
- `get_estimate_purchases(p_estimate_record_id, p_workspace_owner_id)` — план-факт анализ закупок по смете (вкладка «Закупки»), возвращает агрегированные данные из `project_estimate_materials` (план) и `project_estimate_purchases` (факт) с расчётом отклонений и средневзвешенной цены
