...
Все 28 таблиц находятся в схеме `public`, RLS включён на всех. Схема управляется через миграции Supabase (не хранятся локально в репозитории).
...
56 миграций Supabase (от `002_rls_policies` до `049_fix_material_add_response_work_totals`). Хранятся на сервере Supabase, не в репозитории. Основные этапы:
...
| 033–041 | Сметы: записи, секции, работы, материалы, drag-and-drop, дубликаты |
| 042–049 | Уведомления, фиксы estimate editor |
