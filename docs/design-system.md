...
>
> **Последняя проверка:** 2026-05-16 — актуально. Все 35 компонентов `components/ui/` задокументированы. Добавлен Spinner. Фичи без изменений.
...
SVG-спиннер загрузки на основе `animate-spin`. Используется в кнопках при отправке форм, в скелетонах загрузки и любых асинхронных операциях.
