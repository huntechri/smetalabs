...
>
> **Последняя проверка:** 2026-05-23 — актуально. Все 39 компонентов `components/ui/` задокументированы. Изменения: добавлены Alert, AlertDialog, InputGroup; сводные значения в estimate editor мигрировали на стандартные Shadcn outline Badges.
...
SVG-спиннер для индикации загрузки. Использует `currentColor` для наследования цвета текста и стандартный Tailwind-класс `animate-spin`.
...
**Импорт:** `@/components/ui/alert`
**Экспорт:** `Alert`, `AlertTitle`, `AlertDescription`

**Пропсы:**

- `variant` — `"default"` | `"destructive"` (у `Alert`)
...
**Импорт:** `@/components/ui/alert-dialog`
**Экспорт:** `AlertDialog`, `AlertDialogTrigger`, `AlertDialogContent`, `AlertDialogHeader`, `AlertDialogTitle`, `AlertDialogDescription`, `AlertDialogFooter`, `AlertDialogCancel`, `AlertDialogAction`

Модальное окно подтверждения для критических действий (аналог Dialog, но с обязательным подтверждением). Используется для удаления, архивирования и других необратимых операций.
...
**Импорт:** `@/components/ui/input-group`
**Экспорт:** `InputGroup`, `InputGroupAddon`

Группировка input-элементов с addon-ами (иконки, лейблы, кнопки). Композиция через children.

```tsx
// ✅ Input с левой иконкой и правой кнопкой
<InputGroup>
  <Input placeholder="Search..." />
  <Button variant="ghost" size="icon-sm">
    <MagnifyingGlassIcon />
