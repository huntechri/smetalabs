...
### 6.10 Loading states для структурных списков

Для таблиц, каталогов, списков и карточных сеток предпочтительный loading-паттерн — `Skeleton` из `@/components/ui/skeleton`. Skeleton должен повторять структуру будущего контента: основные колонки, группы, размеры строк и общий ритм.

`Spinner` допустим только для маленьких inline/action-состояний или когда форма будущего контента неизвестна. Не создавай локальные SVG-spinner и локальные `animate-spin` loader внутри feature-компонентов.

Для списков с закреплённой нижней пагинацией:

- скроллится только область строк;
- пагинация остаётся видимой;
- кнопки пагинации отключаются во время `loading`/`isFetching`;
- loading показывается внутри области строк;
- не дублируй loading-текст в футере.

`/directories/works` — текущий reference-паттерн: первая загрузка, поиск, пагинация и `Suspense fallback` используют skeleton-строки; spinner в loading-состоянии списка не используется.

---

## 6.11 Проверка: фича workspace-settings
