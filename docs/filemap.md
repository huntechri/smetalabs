...
│   │   │   └── page.tsx
│   │   ├── team/                       # Команда (WorkspaceSettingsView — управление участниками, приглашениями, доменами)
│   │   │   └── page.tsx
...
│   │       └── access/
│   │           └── page.tsx            # Права доступа (PermissionsMatrix из features/access-control)
│   │
...
**Замечание:** Маршрут `/templates/[templateId]` ещё не имеет выделенной фичи — его страница использует inline-вёрстку или заглушку. `/team` использует `WorkspaceSettingsView` из фичи `workspace-settings`. PermissionsMatrix вынесена на отдельную страницу `/settings/access` и доступна через меню пользователя (пункт «Права доступа»).
