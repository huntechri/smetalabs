...
Если материал связан со справочником, при загрузке данных сметы через Left Join с таблицей `directory_materials` подтягивается ссылка на изображение `image_url` (мапится в свойство `imageUrl` во фронтенд-модели). Это изображение используется в UI для визуального отображения материала.

Изменение справочника материалов позже не должно менять уже сохранённую смету (за исключением динамической подгрузки актуального `image_url`).
...
```txt
расход = количество материала / количество работы (при количестве работы > 0)
```
...
Для материалов в возвращаемом `jsonb` поле `imageUrl` извлекается из `directory_materials.image_url` с помощью `LEFT JOIN`. Это обеспечивает синхронизацию изображений материалов при любых изменениях (создании раздела, добавлении работ/материалов, архивации).
