...
export function handleDirectorySuppliersRouteError(
  err: unknown,
  routeLabel: string
) {
  if (err instanceof DirectorySuppliersApiError)
...
export async function handleDirectorySuppliersListRequest(
  request: NextRequest
) {
  try {
    const params = parseDirectorySuppliersListParams(
      request.nextUrl.searchParams
    )
    const response = await listDirectorySuppliers(params)
    return NextResponse.json(response)
  } catch (err) {
    return handleDirectorySuppliersRouteError(
      err,
...
export async function handleDirectorySupplierCreateRequest(
  request: NextRequest
) {
  try {
...
export async function handleDirectorySupplierUpdateRequest(
  request: NextRequest,
  id: string
) {
  try {
