...
import {
  buildSortPayload,
  filterEstimateSections,
  moveItem,
  type MoveDirection,
} from "@/features/estimates/estimate-details/lib/estimate-editor-view-model"
...
export function EstimateEditorView({
  ...
  const visibleSections = React.useMemo(
    () => filterEstimateSections(sectionsForMemo, deferredSearch),
  ...
  const moveSection = useCallback(
    async (sectionId: string, direction: MoveDirection) => {
      ...
      await save({
        ...
        payload: { items: buildSortPayload(next) },
      })
    },
  ...
  const moveWork = useCallback(
    async (sectionId: string, workId: string, direction: MoveDirection) => {
      ...
      await save({
        ...
        payload: { sectionId, items: buildSortPayload(next) },
      })
    },
    [content, searchActive, save]
  )
