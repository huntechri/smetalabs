...
Модуль поддерживает экспорт результатов план-факт анализа в отформатированный файл Excel (`.xlsx`).

* **Файл логики экспорта:** [purchases-excel-exporter.ts](file:///c:/Users/Admin/smetalabs/features/purchases/lib/purchases-excel-exporter.ts)
* **Триггер:** Событие `project-purchases:export`, генерируемое панелью инструментов вкладки.
* **Структура отчета:**
  * **Брендированная шапка:** Отрисовка логотипа подрядчика (`workspaceLogo`) в объединенном блоке ячеек `A2:B5` (при наличии). Вывод реквизитов (Подрядчик, Заказчик, Адрес объекта) и названия сметы.
  * **Колонки таблицы:**
    1. `№` — порядковый номер
    2. `Наименование материала` — широкая текстовая колонка с автовысотой строк
    3. `Ед. изм.` — единица измерения
    4. `План: Кол-во` — плановое количество из сметы
    5. `План: Цена` — плановая цена
    6. `План: Сумма` — расчетная формула (`=D*E`)
    7. `Факт: Кол-во` — фактически закупленное количество
    8. `Факт: Цена` — средневзвешенная фактическая цена
    9. `Факт: Сумма` — расчетная формула (`=G*H`)
    10. `Отклонение` — отклонение в деньгах (`=F-I` с обработкой пустых значений)
  * **Строка итогов:** «ИТОГО ПО ЗАКУПКАМ» с формулами `SUM` для подсчета суммарных плановых и фактических расходов, а также общего отклонения.
  * **Условное форматирование:** Колонка `Отклонение` автоматически окрашивает текст в зеленый цвет (emerald) при экономии (значение > 0) и в красный цвет при перерасходе (значение < 0).
