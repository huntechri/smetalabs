export async function exportPurchasesToExcel(content: {
  record: {
    name: string
  ...
  const worksheet = workbook.addWorksheet("Приложение №1 (Закупки)", {
  ...
  // Title row
  const titleRow = worksheet.addRow(["", "", "ПЛАН-ФАКТНЫЙ АНАЛИЗ ЗАКУПОК"])
  ...
  content.purchases.forEach((purchase: any, idx: number) => {
    ...
    const dataRow = worksheet.addRow([
      ...
      purchase.title,
      purchase.unit,
      purchase.planQuantity ?? 0,
      purchase.planPrice ?? 0,
      { formula: `D${rowIdx}*E${rowIdx}` },
      purchase.factQuantity !== null ? purchase.factQuantity : null,
      purchase.factAvgPrice !== null ? purchase.factAvgPrice : null,
  ...
  const safeName = content.record.name
    ...
    .replace(/\s+/g, "_")
  link.download = `Закупки_Смета_${safeName}.xlsx`
